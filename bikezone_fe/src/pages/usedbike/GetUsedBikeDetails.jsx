...
import { deleteUsedBikeAd, getUsedBikeDetails } from '../../features/usedbike/usedbike.thunk'

const GetUsedBikeDetail = () => {
    const { id } = useParams();
    const dispatch = useDispatch()
    const navigate = useNavigate()

    const { usedBike } = useSelector(state => state.usedBike)
    ...
    const handleWhatsAppClick = () => {
        ...
        const whatsappNumber = usedBike?.contact;
        ...
        if (whatsappNumber) {
            ...
            // Message to pre-fill
            const message = `Hello, I'm interested in renting the ${usedBike?.title} (${usedBike?.model}). Can you provide more information? It is priced at PKR ${usedBike?.price} .`;
                ...
                <section className="py-10 font-poppins bg-gradient-to-bl from-gray-200 via-gray-400 to-gray-600">
                    <div className="max-w-6xl px-4 mx-auto">
                        <div className="flex flex-wrap mb-24 -mx-4">
                            {
                                usedBike && <div className="w-full px-4 mb-8 md:w-1/2 md:mb-0">
                                    <div className="sticky h-full overflow-y-scroll no-scrollbar">
                                        <Carousal images={usedBike?.images} />
                                        <div className="flex-wrap hidden -mx-2 md:flex">
                                            {usedBike?.images && usedBike?.images?.map((item) => (
                                                            ...
                                                            <div className="w-full mb-4 ">
                                                                <div className="flex items-center">
                                                                    ...
                                                                    <p className=" text-sm font-medium text-gray-500 dark:text-gray-400">
                                                                        {`${usedBike?.address}, ${usedBike?.city}`}
                                                                    </p>
                                                                </div>
                                                            </div>
                                                            <div className="w-full mb-4 ">
...
export default GetUsedBikeDetail